...
const int PIN_GRND_MOTOR_EN = 11;
const int PIN_GRND_MOTOR_IN1 = 12;
const int PIN_GRND_MOTOR_IN2 = 13;

const int PIN_BROADCAST_RX = A1;
const int PIN_BROADCAST_TX = A0;

const int PIN_SCALE_RX = A2;
const int PIN_SCALE_TX = A3;
...
//BRD2 Definitions
Executor BRD2("BRD2"),FLIP("FLIP"),GRND("GRND");
State BRD1("BRD1"),PHASE("PHAS"),MODE("MODE"),CON("CON"),ST2("ST2"),STGR("STGR");
State states1[]={BRD1,PHASE,MODE,CON,ST2,STGR};
int NUM_OF_STATES_BRD1=6;
...
//ST2 Declarations
Timer timerFLIP,timerBRD2;
int jiggle_COUNT;
int distance;

//Grinding/Scale area variables
Timer timerGRND;
float scaleWeight;
int i_fill=0;
bool _done;
...
while(!connectionEstablished){
  serialBroadcast.checkAndSend(BRD2);
  serialBroadcast.listenAndUpdate(states1,NUM_OF_STATES_BRD1);
  BRD1=states1[0];
  delay(5);
  ...
  //send section
  //serialBroadcast.checkAndSend(BRD2);
  serialBroadcast.checkAndSend(FLIP);
  serialBroadcast.checkAndSend(GRND);
